...
Let's add a sentence with a couple of references at the end [@alston2021, @cruwell2019, @lindsay2018]. 
According to @cruwell2019, open science is an umbrella term that covers several concepts, including openness, transparency, rigour, reproducibility, replicability, and accumulation of knowledge.
...
Our simulated sample consists of `r length(df$id)` participants. 
`r printnum(sum(df$condition %in% "work"), numerals=FALSE, capitalize=TRUE)` participants were randomly assigned to the workshop condition, while the remaining `r printnum(sum(df$condition %in% "contr"))` participants were randomly assigned to the control condition.
`r printnum(sum(df$gender %in% "male"), numerals=FALSE, capitalize=TRUE)` participants were male, while `r printnum(sum(df$gender %in% "female"))` participants were female.
On average, participants were `r printnum(mean(df$age))` years old ($SD = `r sd(df$age)`$ years).
...
There was a strong, positive correlation between both items ($r = `r printnum(cor(df$consc1, df$consc2))`$), offering support for the reliability of the measure.
...
This code chunk selects a couple of variables from the dataset, then uses the report function from the report package to create a descriptives table, and finally uses the apa_table function from the papaja package to format the table according to APA rules.
...
Condition(`r m1_apa$full$condition`) affected change in positive attitude to open science practices. 
Gender was not related to change in positive attitude to open science practices, `r m1_apa$full$gender`.
There was no significant interaction effect between condition and gender, `r m1_apa$full$condition_gender`.
